...
static TreeNode* simp_expr(TreeNode* tn);
//static ExpType relop(void);
static TreeNode* add_expr(TreeNode* tn);
//static ExpType addop(void);
static TreeNode* term(TreeNode* tn);
//static ExpType mulop(void);
static TreeNode* factor(TreeNode* tn);
...
TreeNode* params(void) // params -> param_list | void
{
	...
	type = type_spec();
	if (type == Void && token == RPAREN) { // void 단독 사용 불가, params -> void
		t = newExpNode(VarDeclK);
		t->isParam = TRUE;
		t->type = Void;
	}
	else {
		t = param_list(type); // params -> param_list
	}
...
TreeNode* simp_expr(TreeNode* tn) //add_expr relop add_expr | add_expr
{
	TreeNode* t;
	TreeNode* q;
	TokenType oper;
...
TreeNode* add_expr(TreeNode* tn) //add_expr addop term | term
{
	TreeNode* t;
	TreeNode* q;
...
TreeNode* term(TreeNode* tn)//term mulop factor | factor
{
	TreeNode* t;
	TreeNode* q;
...
TreeNode* factor(TreeNode* tn) //( expr ) | var | call | NUM
{
	TreeNode* t;
